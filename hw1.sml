...
fun dates_in_months (dates : (int*int*int) list, months : int list) =
  ...
  then []
  else let val x = dates_in_month(dates, hd months)
       in
	   if null x
	   then []
	   else x @ dates_in_months(dates, tl months)
       end
...
fun number_in_months_challenge (dates : (int*int*int) list, months : int list) =
  if null dates orelse null months
  then 0
  else if not (null (tl months)) andalso hd months = hd (tl months)
...
fun dates_in_months_challenge (dates : (int*int*int) list, months : int list) =
  ...
  then dates_in_months_challenge(dates, tl months) 
      ...
      let val x = dates_in_month(dates, hd months)
	   ...
	   if null x
	   then []
	   else x @ dates_in_months_challenge(dates, tl months)
       end
...
fun reasonable_date (date : (int*int*int)) =
  ...
  if #1 date > 0 andalso (#1 date mod 4 = 0 orelse #1 date mod 400 = 0)
  then
      if #2 date > 0 andalso #2 date <= 12
      then #3 date > 0 andalso #3 date <= 29
      else #2 date > 0 andalso #2 date <= 12 
  (* checking for non-leap years *)    
  else if #1 date > 0
  then
      if #2 date > 0 andalso #2 date <= 12 andalso #2 date <> 2
      then #3 date > 0 andalso #3 date <= 31
      else if #2 date = 2
      then #3 date > 0 andalso #3 date <= 28
      else #2 date > 0 andalso #2 date <= 12 
  else #1 date > 0
